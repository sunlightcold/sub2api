...
func (r *channelMonitorRepository) Create(ctx context.Context, m *service.ChannelMonitor) error {
	...
	builder := client.ChannelMonitor.Create().
		...
		SetCheckMode(channelMonitorColumnCheckMode(m.CheckMode))
	if m.TemplateID != nil {
...
func channelMonitorHeadersForPersistence(m *service.ChannelMonitor) map[string]string {
	...
	if mode := defaultCheckModeRepo(m.CheckMode); mode == service.MonitorCheckModeRequest || mode == service.MonitorCheckModePass {
...
func channelMonitorColumnCheckMode(checkMode string) string {
	switch defaultCheckModeRepo(checkMode) {
	case service.MonitorCheckModeQuota:
		return service.MonitorCheckModeQuota
	case service.MonitorCheckModeQuotaProbe:
		return service.MonitorCheckModeQuotaProbe
	default:
