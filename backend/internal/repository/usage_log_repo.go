...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	upstreamTiming := nullStringInt64MapJSON(log.UpstreamTiming)
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	...
	log := &service.UsageLog{
		...
		UpstreamTiming:        service.UsageUpstreamTiming(stringInt64MapFromNullJSON(upstreamTiming)),
		CreatedAt:             createdAt,
	}
...
func nullStringInt64MapJSON(v map[string]int64) any {
	if len(v) == 0 {
		return nil
	}
	payload, err := json.Marshal(v)
...
func stringInt64MapFromNullJSON(v sql.NullString) map[string]int64 {
	if !v.Valid || strings.TrimSpace(v.String) == "" {
		return nil
	}
