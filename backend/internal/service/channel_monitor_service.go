...
func (s *ChannelMonitorService) decryptAPIKeyForDuplicate(source *ChannelMonitor) (string, error) {
	...
	if strings.TrimSpace(plain) == "" {
		mode := defaultCheckMode(source.CheckMode)
		if monitorCheckModeUsesQuota(mode) || mode == MonitorCheckModePass {
			return "", nil
		}
...
func validateCreateParams(p ChannelMonitorCreateParams) error {
	...
	// quota/pass 模式 endpoint/api_key 留空，避免要求用户填无意义的占位值。
	if checkMode != MonitorCheckModeQuota && checkMode != MonitorCheckModePass {
...
func validateMonitorModeFields(m *ChannelMonitor) error {
	...
	if checkMode != MonitorCheckModeQuota && checkMode != MonitorCheckModePass && strings.TrimSpace(m.Endpoint) == "" {
...
func (s *ChannelMonitorService) validateProbeAPIKey(m *ChannelMonitor, newPlainKey string) error {
	...
	if mode == MonitorCheckModeQuota || mode == MonitorCheckModePass {
		return nil
	}
...
func (s *ChannelMonitorService) RunCheck(ctx context.Context, id int64) ([]*CheckResult, error) {
	...
	checkMode := defaultCheckMode(m.CheckMode)
	if checkMode != MonitorCheckModeQuota && checkMode != MonitorCheckModePass && m.APIKeyDecryptFailed {
...
func (s *ChannelMonitorService) runChecksConcurrent(ctx context.Context, m *ChannelMonitor) []*CheckResult {
	...
	var pingMs *int
	if defaultCheckMode(m.CheckMode) == MonitorCheckModePass {
