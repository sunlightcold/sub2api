...
func validateCheckMode(provider, checkMode string) error {
	...
	case MonitorCheckModeProbe, MonitorCheckModeQuota, MonitorCheckModeQuotaProbe,
		MonitorCheckModeRequest, MonitorCheckModePass:
	default:
		return ErrChannelMonitorInvalidCheckMode
	}
	if checkMode != MonitorCheckModeQuota && checkMode != MonitorCheckModePass && !providerSupportsProbe(provider) {
		return ErrChannelMonitorInvalidCheckMode
	}
	return nil
}
...
func normalizePassLatencyRange(minMs, maxMs int) (int, int) {
	if minMs == 0 {
		minMs = MonitorDefaultPassLatencyMinMs
	}
	if maxMs == 0 {
		maxMs = MonitorDefaultPassLatencyMaxMs
	}
	return minMs, maxMs
}
