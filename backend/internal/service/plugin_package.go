...
func (i *PluginPackageInstaller) Install(ctx context.Context, reader io.Reader, installedBy *int64) (*PluginInstallation, error) {
	...
	// Windows does not allow renaming the archive while zip.OpenReader still
	// holds its file handle. Extraction is complete, so release it before the
	// package and install directory are committed.
