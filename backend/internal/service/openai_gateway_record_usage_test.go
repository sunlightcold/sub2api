...
func TestOpenAIGatewayServiceRecordUsage_GPT56LongContextPricingFollowsGroupSwitch(t *testing.T) {
	tests := []struct {
		name                       string
		longContextPricingEnabled  bool
		wantInputPricePerToken     float64
		wantOutputPricePerToken    float64
		wantCacheReadPricePerToken float64
	}{
		{
			name:                       "disabled uses base prices",
			longContextPricingEnabled:  false,
			wantInputPricePerToken:     5e-6,
			wantOutputPricePerToken:    30e-6,
			wantCacheReadPricePerToken: 0.5e-6,
		},
		{
			name:                       "enabled applies long context tier",
			longContextPricingEnabled:  true,
			wantInputPricePerToken:     10e-6,
			wantOutputPricePerToken:    45e-6,
			wantCacheReadPricePerToken: 1e-6,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			usage := OpenAIUsage{
				InputTokens:          275048,
				OutputTokens:         270,
				CacheReadInputTokens: 268032,
			}

			err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
				Result: &OpenAIForwardResult{
					RequestID: "resp_gpt56_group_long_context_switch",
					Usage:     usage,
					Model:     "gpt-5.6-sol",
					Duration:  time.Second,
				},
				APIKey: &APIKey{
					...
					Group: &Group{
						ID:                        4057,
						LongContextPricingEnabled: tt.longContextPricingEnabled,
					},
				},
				User:    &User{ID: 2057},
			...
			require.NoError(t, err)
			require.NotNil(t, usageRepo.lastLog)
			require.InDelta(t, float64(7016)*tt.wantInputPricePerToken, usageRepo.lastLog.InputCost, 1e-12)
			require.InDelta(t, float64(usage.OutputTokens)*tt.wantOutputPricePerToken, usageRepo.lastLog.OutputCost, 1e-12)
			require.InDelta(t, float64(usage.CacheReadInputTokens)*tt.wantCacheReadPricePerToken, usageRepo.lastLog.CacheReadCost, 1e-12)
		})
	}
}
