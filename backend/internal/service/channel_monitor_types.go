...
type ChannelMonitor struct {
	...
	CheckMode            string // probe/quota/quota_probe（官方）或 request/pass（兼容已有 fork 配置）
	PassLatencyMinMs     int
	PassLatencyMaxMs     int
	PassPingLatencyMinMs int
	PassPingLatencyMaxMs int
...
type ChannelMonitorCreateParams struct {
	...
	// 官方配额模式：CheckMode 空串默认 probe；quota/quota_probe 必须关联账号。
	AccountID *int64
}
