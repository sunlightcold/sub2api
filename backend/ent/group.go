...
type Group struct {
	...
	// 是否禁用该分组的 Responses API；NULL/false 保持现有行为
	DisableResponsesAPI *bool `json:"disable_responses_api,omitempty"`
	// 是否禁用该分组的 Chat Completions API；NULL/false 保持现有行为
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case group.FieldModelRouting, group.FieldSupportedModelScopes, group.FieldMessagesDispatchModelConfig, group.FieldModelsListConfig:
			values[i] = new([]byte)
		case group.FieldLongContextPricingEnabled, group.FieldPeakRateEnabled, group.FieldIsExclusive, group.FieldAllowImageGeneration, group.FieldAllowBatchImageGeneration, group.FieldImageRateIndependent, group.FieldImageBillingUseRequestedCount, group.FieldVideoRateIndependent, group.FieldClaudeCodeOnly, group.FieldModelRoutingEnabled, group.FieldMcpXMLInject, group.FieldAllowMessagesDispatch, group.FieldDisableResponsesAPI, group.FieldDisableChatCompletionsAPI, group.FieldRequireOauthOnly, group.FieldRequirePrivacySet:
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldDisableResponsesAPI:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field disable_responses_api", values[i])
			} else if value.Valid {
				_m.DisableResponsesAPI = new(bool)
				*_m.DisableResponsesAPI = value.Bool
			}
		case group.FieldDisableChatCompletionsAPI:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field disable_chat_completions_api", values[i])
			} else if value.Valid {
				_m.DisableChatCompletionsAPI = new(bool)
				*_m.DisableChatCompletionsAPI = value.Bool
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	if v := _m.DisableResponsesAPI; v != nil {
		builder.WriteString("disable_responses_api=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
	if v := _m.DisableChatCompletionsAPI; v != nil {
		builder.WriteString("disable_chat_completions_api=")
		builder.WriteString(fmt.Sprintf("%v", *v))
	}
	builder.WriteString(", ")
