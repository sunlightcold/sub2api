...
const usesProbePart = computed(() =>
  form.check_mode === CHECK_MODE_PROBE || form.check_mode === CHECK_MODE_QUOTA_PROBE,
)
const usesModelChecks = computed(() => form.check_mode !== CHECK_MODE_QUOTA)
...
function buildPayload(): CreateParams {
  return {
    ...
    primary_model: usesModelChecks.value ? form.primary_model.trim() : 'quota',
    extra_models: usesModelChecks.value ? form.extra_models : [],
...
async function handleSubmit() {
  ...
  if (usesModelChecks.value && !form.primary_model.trim()) {
